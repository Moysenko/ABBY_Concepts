...
class Processor:
    ...
    def _read_int(self, first, second):
        self._binary.set_int(first, int(input("int> ")))
    ...
    def _push(self, first, second):
        ...
        logger.debug(f'Pushing {first} on stack, val = {val}')
    ...
    def _jmp_zero(self, first, second):
        val = self._binary.get_int(second)
        if val == 0:
